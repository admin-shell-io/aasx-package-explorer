...
namespace AasxToolkit
{
    public static class Program
    {
        static void Main(string[] args)
        {
            ...
            var commandLineParsing = Cli.Parse(cmdLine, args);
            if (commandLineParsing.Errors != null)
            {
                var errorMsg = Cli.FormatParsingErrors(
                    args, commandLineParsing.AcceptedArgs, commandLineParsing.Errors);
                Console.Error.WriteLine(errorMsg);
                System.Environment.ExitCode = -1;
                return;
            }

            var returnCode = Cli.Execute(commandLineParsing.Instructions);
            if (returnCode == null)
            {
            ...
            {
                System.Environment.ExitCode = returnCode.Value;
            }
        }
    }
}