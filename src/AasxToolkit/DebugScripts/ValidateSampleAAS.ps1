...
function Main
{
    Get-ChildItem -File -Path ..\..\..\sample-aasx -Filter *.aasx | Foreach-Object {
		...
		$cmd = ".\..\bin\Debug\net461\win-x64\AasxToolkit.exe load `"$($_.Fullname)`" check+fix save sample.xml validate sample.xml"
		Write-Host ""
		Write-Host -ForegroundColor Yellow "Executing: $cmd"
		Invoke-Expression $cmd

		$cmd = ".\..\bin\Debug\net461\win-x64\AasxToolkit.exe load `"$($_.Fullname)`" check+fix save sample.json validate sample.json"
		Write-Host ""
		Write-Host -ForegroundColor Yellow "Executing: $cmd"
		Invoke-Expression $cmd
	}
